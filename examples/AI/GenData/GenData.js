...
async function post_copyData(req,res) {
    pipeline = [{$set:{reviews:"$$REMOVE"}},{"$out":{db:"vectorTest",coll:"Properties"}}]
    let rval = await sourceData.aggregate(pipeline).toArray()
    res.send(rval)
}

async function get_copyData(req,res) {
    res.send(await destData.countDocuments({}))
}

async function post_vectorize(req,res) {
    let notDoneYet =  destData.find({embedding:null}).limit(1000)
    let c=0
    for await( const doc of notDoneYet)
   {
       if(doc.description && doc.description.length > 0) {
       var vectorRepresentation = await aiService.textToVector(doc.description)
       await destData.updateOne({_id:doc._id},{$set:{embedding:vectorRepresentation}})
       }
       c++;
    }
    res.send("done: "+c);
...
async function createVectorIndex() {
    ...
    try {
        const index = await destData.createSearchIndex("vectorIndex", indexDefinition)
...
async function initWebService() {
    ...
    sourceData = vacationTypeCollection = mongoClient
        .getDatabase("sample_airbnb")
        .getCollection("listingsAndReviews");
        
    destData = mongoClient
        .getDatabase("vectorTest")
